...
def setupDataFrames(dateFr=None, dateTo=None):
    ...
    df["YearSeas"] = [
        f"{yr}-s" if mth > 5 else f"{yr}-w" for mth, yr in zip(df["month"], df["year"])
    ]
...
def fillMinMax(df):
    ...
    dfmin["annote"] = [
        makeAnnote(x, y, "red", 40) for (x, y) in zip(dfmin["date"], dfmin["lv"])
    ]
    dfmax["annote"] = [
        makeAnnote(x, y, "green", -40) for (x, y) in zip(dfmax["date"], dfmax["lv"])
    ]
...
def rangeButtons(steps=None):
    ...
    for s in steps:
        aDic = dict(
            count=s, label=f"{s}y", step="year", stepmode="backward", visible=True
        )
        aList.append(aDic)
    return aList
...
def drawKinGraph():
    ...
    fig = px.scatter(title=chartTitle, labels={"x": "Date", "y": "m below Sea Level"})
    ...
    lines = [
        drawLevel(upperRedLine, "Blue", df),
    ...
    dfRH["ypos"] = [
        histMin + 0.18 if x % 2 == 0 else histMin + 0.1 for x in dfRH["hebyear"]
    ]
                ...
                buttons=list(
                    [
                        ...
                        dict(
                            ...
                            args=[
                                {"visible": []},
                                {"title": chartTitle, "annotations": upAn},
                            ],
                        ),
                        dict(
                            label="Low",
                            ...
                            args=[
                                {"visible": []},
                                {"title": chartTitle, "annotations": dnAn},
                            ],
                        ),
                        dict(
                            label="All",
                            ...
                            args=[
                                {"visible": []},
                                {"title": chartTitle, "annotations": upAn + dnAn},
                            ],
                        ),
                    ]
                ),
                ...
                buttons=list(
                    [
                        dict(
                            label="None ",
                            method="update",
                            args=[{"visible": []}, {"title": chartTitle, "shapes": []}],
                        ),
                        dict(
                            label="Level Lines",
                            method="update",
                        ...
                        dict(
                            ...
                            args=[
                                {"visible": []},
                                {"title": chartTitle, "shapes": lines + rhSh},
                            ],
                        ),
                    ]
                ),
    ...
    fig.update_yaxes(range=[histMin - 0.1, upperRedLine + 0.1], fixedrange=False)
    fig.update_layout(legend=dict(yanchor="top", y=0.99, xanchor="left", x=0.01))
    fig = addRangeSlider(fig, df)

    fig.update_layout(xaxis_title=xAxesTitle, yaxis_title=yAxesTitle)
    ...
    pio.write_html(fig, file=fileHtml, auto_open=True)
    fig.write_image(filePng, engine="kaleido", width=1920, height=1080)
    # chartStudioCreds()
...
def drawChangesGraph(df=None, period=7):
    ...
    figch.update_layout(
        title=titleTxt, legend=dict(x=0.90), yaxis_title=f"{period} Day Change (cm)"
    )
    ...
    for p in periods:
        ...
        thisBut = go.layout.updatemenu.Button(
            label=f"{p} Day",
            method="update",
            args=[{"visible": matrix[i]}, {"title": titleTxt}],
        )
    ...
    figch.update_traces(visible=True, selector=dict(meta=1))
    ...
    pio.write_html(figch, file=fileHtml, auto_open=True)
    figch.write_image(filePng, engine="kaleido", width=1920, height=1080)
...
def addChangeTriangles(figch, plotLevel=True, df=None, period=7):
    ...
    df["hovtext"] = [
        f"{period} day change<br>{lv:.3f}m {ch:.1f}cm<br>{d:%d %b %Y}<extra></extra>"
        for (lv, ch, d) in zip(round(df["level"], 3), df[dCol], df.index)
    ]
    ...
    figch.add_trace(
        go.Scatter(
            ...
            marker=dict(
                ...
                color=colU,
                colorbar=dict(
                    x=0.98, y=0.73, len=0.5, title=f"{period} Day<br>Change<br>(cm)"
                ),
            ),
        )
    )
    figch.add_trace(
        go.Scatter(
            ...
            showlegend=False,
            mode="markers",
            marker=dict(size=mSize, color="white", line=dict(color="#0036B2", width=2)),
        )
    )
    figch.add_trace(
        go.Scatter(
            ...
            marker=dict(
                ...
                showscale=True,
                color=colD,
                colorbar=dict(x=0.98, y=0.23, len=0.5),
            ),
        )
    )
    if plotInterpolated:
        ...
        figch.add_trace(
            go.Scatter(
                ...
                mode="markers",
                marker=dict(
                    size=mSize * 0.5, color="white", line=dict(color="#0036B2", width=2)
                ),
            )
        )
    return figch
